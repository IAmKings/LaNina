...
export class DailyBriefError extends Error {
  constructor(
    ...
    message: string,
    readonly details: Readonly<{ expectedFreezeKey: string | null; currentFreezeKey: string | null }> | null = null,
  ) {
    super(message);
...
export interface DailyBriefMutation extends Omit<DailyBriefFreezeCommand, "targets"> {
...
function validateCommand(command: DailyBriefFreezeCommand): DailyBriefFreezeCommand {
  if (typeof command !== "object" || command === null || Array.isArray(command)) {
    throw new DailyBriefError("VALIDATION", "每日判定命令必须是对象");
  }
  const expectedKeys = [
    "cutoff", "targets", "headline", "summary", "topChanges",
    "actor", "reason", "occurredAt", "expectedFreezeKey",
  ];
  ...
  if (!Array.isArray(command.targets) || command.targets.length !== REQUIRED_DAILY_THESIS_IDS.length) {
    throw new DailyBriefError("VALIDATION", "每日判定必须精确指定六条论点版本");
  }
  ...
  if (REQUIRED_DAILY_THESIS_IDS.some((id) => !seenTheses.has(id))) {
