...
| 发布审核 | `enabled`/`public` 变更、署名、Access 与发布窗口 | **附条件批准（论点发布）**：五条论点（ENSO-CORE-01 / RUBBER-TH-01 / PALM-SEA-01 / MAIZE-SA-01 / SHIP-USEC-01）按 PRD §8.6 / §18 口径允许发布——覆盖缺口必须显示"数据覆盖不足"并按已签字 `coverageGapCap` 封顶置信度，**不用代理、不改数据**；SHIP-EU-01 因结构性无方向证据保持关闭，按路径① 在日报中显式豁免并公开缺口。来源 `enabled`/`public` 变更仍逐项走本行。 | 2026-09-25 | 负责人 D-A 指令（本次会话） | 论点发布放行；来源 `enabled`/`public` 变更仍 `pending` |
